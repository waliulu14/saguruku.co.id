...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($content) || empty($blog_link)) {
        ...
        if (isset($_FILES["image"]) && $_FILES["image"]["error"] === 0) {
            ...
            if ($uploadOk == 0) {
                ...
                if ($result) {
                    ...
                    if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
                        // Update the blog detail entry with the new image_url
                        $update_sql = "UPDATE blog_details SET title = '$title', image_url = '$image_url', content = '$content', blog_link = '$blog_link' WHERE id = $id";
                        if (mysqli_query($conn, $update_sql)) {
                            // Data updated successfully, redirect to the blog details page
            ...
            // If no image file is uploaded, update the blog detail entry without changing the image_url
            $update_sql = "UPDATE blog_details SET title = '$title', content = '$content', blog_link = '$blog_link' WHERE id = $id";
            if (mysqli_query($conn, $update_sql)) {
                // Data updated successfully, redirect to the blog details page
                header('Location: our-blog.php');
                exit();
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "SELECT id, title, image_url, content, blog_link FROM blog_details WHERE id = $id";
        $result = mysqli_query($conn, $sql);
        if ($result) {
            $row = mysqli_fetch_assoc($result);
            $title = $row['title'];
...
<div class="container-fluid" id="container-wrapper">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Edit Blog Detail</h1>
    </div>
                ...
                <div class="card-body">
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="blog_link" name="blog_link" value="<?php echo $blog_link; ?>" required>
                        </div>
